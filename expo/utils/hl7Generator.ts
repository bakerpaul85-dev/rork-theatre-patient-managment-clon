...
export const generateHL7Message = (form: FormData): string => {
  ...
  if (form.caseNumber) {
    segments.push(generateOBXSegment(obxCounter++, 'CASE_NUMBER', form.caseNumber || ''));
  }

  if (form.gender) {
    segments.push(generateOBXSegment(obxCounter++, 'GENDER', form.gender || ''));
  }

  if (form.patientAddress) {
    segments.push(generateOBXSegment(obxCounter++, 'PATIENT_ADDRESS', form.patientAddress || ''));
  }

  if (form.homePhone) {
    segments.push(generateOBXSegment(obxCounter++, 'HOME_PHONE', form.homePhone || ''));
  }

  if (form.workPhone) {
    segments.push(generateOBXSegment(obxCounter++, 'WORK_PHONE', form.workPhone || ''));
  }

  if (form.ward) {
    segments.push(generateOBXSegment(obxCounter++, 'WARD', form.ward || ''));
  }

  if (form.bed) {
    segments.push(generateOBXSegment(obxCounter++, 'BED', form.bed || ''));
  }

  if (form.admissionDate) {
    segments.push(generateOBXSegment(obxCounter++, 'ADMISSION_DATE', form.admissionDate || ''));
  }

  if (form.admissionTime) {
    segments.push(generateOBXSegment(obxCounter++, 'ADMISSION_TIME', form.admissionTime || ''));
  }

  if (form.authorizationCode) {
  ...
  if (form.dap) {
    segments.push(generateOBXSegment(obxCounter++, 'DAP', form.dap || ''));
  }
